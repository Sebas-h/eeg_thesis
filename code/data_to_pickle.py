...
def pickle_data(data_folder, subject_id, low_cut_hz):
    ...
    train_cnt = mne_apply(
        lambda a: exponential_running_standardize(a.T, factor_new=factor_new, init_block_size=init_block_size,
                                                  eps=1e-4).T, train_cnt)

    # same thing for test
    test_cnt = test_cnt.drop_channels(['STI 014', 'EOG-left', 'EOG-central', 'EOG-right'])
    ...
    with open(f'{subject_id}_train_set.pickle', 'wb') as f:
        pickle.dump(train_set, f, pickle.HIGHEST_PROTOCOL)
    with open(f'{subject_id}_valid_set.pickle', 'wb') as f:
        pickle.dump(valid_set, f, pickle.HIGHEST_PROTOCOL)
    with open(f'{subject_id}_test_set.pickle', 'wb') as f:
...
if __name__ == '__main__':
    ...
    data_folder = '/Users/sebas/code/eeg_datasets/BCICIV_2a_gdf/'
    low_cut_hz = 4  # 0 or 4

    for subject_id in range(1, 10):
        pickle_data(data_folder, subject_id, low_cut_hz)
        log.info(f'{subject_id} pickled!')
