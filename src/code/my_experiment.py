...
def run_exp(data_folder, subject_id, low_cut_hz, model, cuda, pickle_path=None):
    ...
    if model == 'shallow':
        ...
        to_dense_prediction_model(model)  # needed from cropped training data
    elif model == 'deep':
        model = Deep4Net(n_chans, n_classes, input_time_length=input_time_length,
                            final_conv_length=2).create_network()
        to_dense_prediction_model(model)  # needed from cropped training data
    elif model == 'eegnet':
        model = EEGNetv4(n_chans, n_classes, input_time_length=train_set.X.shape[2]).create_network()
        cropped = False
...
if __name__ == '__main__':
    ...
    pickle_data_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data')) + \
        "/bcic_iv_2a_all_9_subjects.pickle"
    subject_id = 1 # 1-9
    low_cut_hz = 4 # 0 or 4
    model = 'eegnet' #'shallow' or 'deep'
    cuda = False
    exp = run_exp(data_folder, subject_id, low_cut_hz, model, cuda, pickle_path=pickle_data_path)
    # Save expirement:
