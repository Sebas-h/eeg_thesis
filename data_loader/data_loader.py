...
from braindecode.datautil.splitters import concatenate_sets, split_into_train_test
import numpy as np
from base.base_data_loader import BaseDataLoader
from data_loader.util.pair_data import (
...
def get_dataset(subject_id, i_valid_fold, dataset_name, config):
    ...
    if config["experiment"]["type"] == "ccsa_da":
        source_subject_ids = [i for i in range(1, n_subjects + 1) if i != subject_id]
        return PairedProcessedData(
            subject_id, source_subject_ids, n_folds, i_valid_fold, data_dir, n_classes
        )
    else:
...
class PairedProcessedData(BaseDataLoader):
    def __init__(
        ...
        tgt_full_train_set, _ = _load_data(data_dir, target_subject_id)

        src_full_train_set, _ = _load_data(data_dir, list_source_subject_ids)
        # Create paired dataset
        paired_full_train_set = create_paired_dataset(
            tgt_full_train_set, src_full_train_set, n_classes
        )
        # Split into train and valid sets
        train_set, valid_set = split_paired_into_train_test(
            paired_full_train_set, n_folds, i_valid_fold, n_classes
        )
        super().__init__(train_set, valid_set, None, n_classes)


class ProcessedDataset(BaseDataLoader):
    def __init__(self, subject_ids, n_folds, i_valid_fold, data_dir, n_classes):
        # Load data
        full_train_set, test_set = _load_data(data_dir, subject_ids)
        # Split into train and valid sets
        train_set, valid_set = split_into_train_test(
            full_train_set, n_folds, i_valid_fold
        )
...
def _load_data(data_dir, subject_ids):
    assert (
        type(subject_ids) is list or type(subject_ids) is int
    ...
    test_set = _load_and_merge_data([f"{data_dir}/{i}_test.h5" for i in subject_ids])
...
def _load_h5_data(file_path):
    ...
    with h5py.File(file_path, "r") as h5file:
        ...
        return SignalAndTarget(list(h5file[keys[0]][()]), list(h5file[keys[1]][()]))
